...
function wpgcp_PrintCells($params)
{
	...
	<?php
		...
		ImageTTFText($im, $font_size = 20, $angl = 0, $x = 68 - (20*($params['captcha_length'] - 3) + 12 * ($cell_size - 3)), $y = 42, $textcolor, $ttf_file, $click_nums);
		...
		for ($i = 1; $i <= $cell_size*$cell_size; $i++)
		{
			...
			$im = imagecreate($cell_css_size, $cell_css_size); 						$bg = imagecolortransparent($im, imagecolorallocate($im, 20, 0, 0));
						...
						ImageTTFText($im, $font_size = rand(30-intval($cell_margin/2),45-intval($cell_margin/2)), $angl = rand(-5,5), $x = rand(30-$cell_margin, 35-$cell_margin), $y = rand(60-$cell_margin, 65-$cell_margin), $textcolor, $ttf_file, $i);
